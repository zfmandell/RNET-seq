...
main() {
    ...
    then
        ...
        bwa mem -t 6 /NC_000913.2.fasta trimmed.fq.gz > sample.sam 
        ...
        kallisto index -i index /NC_000913.2.transcriptome.fasta
        ...
        base_two=$base$two
        base=$output_prefix
        three='.sorted.bam.bai'
        base_three=$base$three
        cat sample.sorted.bam.bai > $base_three
        cat sample.sorted.bam > $base_one
        cat ./results/abundance.tsv > $base_two
    fi
    if [ "$organism" = "Bacillus subtilis - NC_000964.3" ] 
    then
        java -jar /trimmomatic-0.39.jar SE -threads 6 -phred33 FASTQ.fastq.gz trimmed.fq.gz ILLUMINACLIP:/trimseq.fasta:2:30:7 MINLEN:20 SLIDINGWINDOW:5:25
        bwa mem -t 6 /NC_000964.3.fasta trimmed.fq.gz > sample.sam 
        samtools view -bS sample.sam  > sample.bam 
        samtools sort -o sample.sorted.bam sample.bam
        samtools index sample.sorted.bam
        gunzip -c trimmed.fq.gz > fastq_unzip.fq
        stats=$(awk 'BEGIN { t=0.0;sq=0.0; n=0;} ;NR%4==2 {n++;L=length($0);t+=L;sq+=L*L;}END{m=t/n;printf("%f %f",m,sq/n-m*m);}' fastq_unzip.fq)
        IFS=' ' read mean sd <<< $stats
        kallisto index -i index /NC_000964.3.transcriptome.fasta
        kallisto quant -i index -o results --single --rf-stranded -l $mean -s $sd fastq_unzip.fq
        base=$output_prefix
        one='.sorted.bam'
        base_one=$base$one
    ...
    BAM=$(dx upload $base_one --brief)
    BAMi=$(dx upload $base_three --brief)
    ...
    dx-jobutil-add-output BAMi "$BAMi" --class=file
