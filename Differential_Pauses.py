        ...
        for line in inp:
            if len(line.strip().split('\t')) > 1:
                if line.strip().split('\t')[2] == 'gene':
                    ...
                    synonyms = line.strip().split('\t')[8].split('=')[7].split(';')[0].split(',')
                    return_dict[gene] = [start,end,synonyms]
                    if strand == '+':
                        CDS_fwd[start] = end
                    else:
                        CDS_rev[end] = start
...
def pause_rank(pauses_WT,pauses_Mutant,wig_WT,wig_Mutant,Median_Threshold):
    ranked = {}
    for key,value in pauses_WT.items():
        ...
        if abs(Median_WT) > Median_Threshold and abs(Median_Mutant) > Median_Threshold:
...
def alt_median_calc(key,strand,wig_WT,wig_Mutant,upstream,downstream):
    if strand == '+':
        return np.median(wig_WT[key-upstream:key+downstream+1]),np.median(wig_Mutant[key-upstream:key+downstream+1])
    else:
        return np.median(wig_WT[key-downstream:key+upstream+1]),np.median(wig_Mutant[key-downstream:key+upstream+1]) 

def printer_l(name,ranked,genes_WT,genes_Mutant,WT_fwd,WT_rev,Mutant_fwd,Mutant_rev,GFF3,TU_fwd,TU_rev,CDS_fwd,CDS_rev,TU_scaling_wt,TU_scaling_mut,CDS_scaling_wt,CDS_scaling_mut,genome,upstream,downstream,WT_lengths,Mutant_lengths,upstream_normalization,downstream_normalization,WT_wig_fwd,WT_wig_rev,Mutant_wig_fwd,Mutant_wig_rev):
    with open(name,'w') as outp:
        outp.write('coord,strand,gene,offset,AKA,PlusOne,sense,WT CDS TPM,Mutant CDS TPM,Log2FC-CDS TPM,WT TU TPM,Mutant TU TPM,Log2FC-TU TPM,TSS,TTS,start,stop,dist TSS,dist TTS,dist start,dist stop,WT Count,Mutant Count,WT Median,Mutant Median,WT Score,Mutant Score,log2FC-Pause,log2FC-Pause-TPM,alt WT Median,alt WT Score,alt Mutant Median,alt Mutant Score,alt log2FC-Pause,alt log2FC-Pause-TPM,Diff-short,short-WT,short-Mutant,Diff-16,16-WT,16-Mutant,Diff-17,17-WT,17-Mutant,Diff-18,18-WT,18-Mutant,Diff-long,long-WT,long-Mutant,sequence,threshold\n')
        for key,value in ranked.items():
            ...
            try:
                ...
                new_gene = gene_translate(gene,GFF3)
                start,stop = pause_place_GFF(new_gene,strand,GFF3)
            ...
            if strand == '+':
                alt_WT_Median,alt_Mutant_Median = alt_median_calc(coord,strand,WT_wig_fwd,Mutant_wig_fwd,upstream_normalization,downstream_normalization)
            else:
                alt_WT_Median,alt_Mutant_Median = alt_median_calc(coord,strand,WT_wig_rev,Mutant_wig_rev,upstream_normalization,downstream_normalization)
            ...
            WT_TU_TPM = round(TPM_calc(coord,strand,TU_fwd,TU_rev,WT_fwd,WT_rev,TU_scaling_wt),0)
            Mutant_TU_TPM = round(TPM_calc(coord,strand,TU_fwd,TU_rev,Mutant_fwd,Mutant_rev,TU_scaling_mut),0)
            ...
            if WT_TU_TPM > 0 and Mutant_TU_TPM > 0:
                Log2FC_TU = round(np.log2(Mutant_TU_TPM/WT_TU_TPM),2)
            else:
                Log2FC_TU = 'NA'
            ...
            if WT_CDS_TPM != 0 and Mutant_CDS_TPM != 0 and WT_Score > 0 and Mutant_Score > 0:
                Log2FC_N = round(np.log2((Mutant_Score/Mutant_CDS_TPM)/(WT_Score/WT_CDS_TPM)),2)
            else:
                Log2FC_N = 'NA'

            if alt_WT_Median > 0 and alt_Mutant_Median > 0:
                alt_WT_Score = round(WT_counts/alt_WT_Median,2)
                alt_Mutant_Score = round(Mutant_counts/alt_Mutant_Median,2)
                if alt_WT_Score > 0 and alt_Mutant_Score > 0:
                    alt_log2FC = round(np.log2(alt_Mutant_Score/alt_WT_Score))
                else:
                    alt_log2FC = 'NA'
                if WT_CDS_TPM != 0 and Mutant_CDS_TPM != 0 and alt_WT_Score > 0 and alt_Mutant_Score > 0:
                    alt_Log2FC_N = round(np.log2((alt_Mutant_Score/Mutant_CDS_TPM)/(alt_WT_Score/WT_CDS_TPM)),2)
                else:
                    alt_Log2FC_N = 'NA'
            else:
                alt_WT_Median = 'NA'
                alt_Mutant_Median = 'NA'
                alt_WT_Score = 'NA'
                alt_Mutant_Score = 'NA'
                alt_log2FC = 'NA'
                alt_Log2FC_N = 'NA'

            if WT_TU_TPM != 0 and Mutant_TU_TPM != 0 and WT_CDS_TPM != 0 and Mutant_CDS_TPM != 0:
                outp.write(str(coord)+','+strand+','+gene+','+offset+','+AKA+','+plus1+','+sense+','+str(WT_CDS_TPM)+','+str(Mutant_CDS_TPM)+','+str(Log2FC_CDS)+','+str(WT_TU_TPM)+','+str(Mutant_TU_TPM)+','+str(Log2FC_TU)+','+\
                str(TSS)+','+str(TTS)+','+str(start)+','+str(stop)+','+str(dTSS)+','+str(dTTS)+','+str(dstart)+','+str(dstop)+','+\
                str(WT_counts)+','+str(Mutant_counts)+','+str(WT_Median)+','+str(Mutant_Median)+','+str(WT_Score)+','+str(Mutant_Score)+','+str(Log2FC)+','+str(Log2FC_N)+','+\
                str(alt_WT_Median)+','+str(alt_WT_Score)+','+str(alt_Mutant_Median)+','+str(alt_Mutant_Score)+','+str(alt_log2FC)+','+str(alt_Log2FC_N)+','+\
...
def printer_s(name,ranked,genes_WT,genes_Mutant,WT_fwd,WT_rev,Mutant_fwd,Mutant_rev,GFF3,TU_fwd,TU_rev,CDS_fwd,CDS_rev,TU_scaling_wt,TU_scaling_mut,CDS_scaling_wt,CDS_scaling_mut,genome,upstream,downstream,WT_lengths,Mutant_lengths,upstream_normalization,downstream_normalization):
    with open(name,'w') as outp:
        outp.write('coord,strand,gene,offset,AKA,PlusOne,sense,WT CDS TPM,Mutant CDS TPM,Log2FC-CDS TPM,WT TU TPM,Mutant TU TPM,Log2FC-TU TPM,TSS,TTS,start,stop,dist TSS,dist TTS,dist start,dist stop,WT Count,Mutant Count,WT Median,Mutant Median,WT Score,Mutant Score,log2FC-Pause,log2FC-Pause-TPM,Diff-short,short-WT,short-Mutant,Diff-16,16-WT,16-Mutant,Diff-17,17-WT,17-Mutant,Diff-18,18-WT,18-Mutant,Diff-long,long-WT,long-Mutant,sequence,threshold\n')
        for key,value in ranked.items():
            coord = int(key.split('_')[0])
            strand = str(key.split('_')[1])
...
def main(TSS, TTS, GFF3, genome, WT_Pauses, Mutant_Pauses, WT_Raw, Mutant_Raw, WT_Sizes, Mutant_Sizes, WT_Coverage_Fwd, WT_Coverage_Rev, Mutant_Coverage_Fwd, Mutant_Coverage_Rev, upstream_sequence, downstream_sequence, upstream_normalization, downstream_normalization, Median_Threshold, Output_Name_Prefix):
    ...
    for gene in genes_WT_list:
        try:
            ...
            if gene_translate(gene,GFF3) == None:
    ...
    #rank all pauses
    ranked = pause_rank(pauses_WT,pauses_Mutant,WT_wig,Mutant_wig,Median_Threshold)
    ...
    TU_wt,TU_mut = TPM_scaling(TU_fwd,TU_rev,WT_fwd,WT_rev),TPM_scaling(TU_fwd,TU_rev,Mutant_fwd,Mutant_rev)
    ...
    if upstream_normalization !=0 or downstream_normalization !=0:
        WT_wig,Mutant_wig = read_wig_count("WT_Raw"),read_wig_count("Mutant_Raw")
        WT_wig_fwd = [abs(x) if x>0 else 0 for x in WT_wig]
        WT_wig_rev = [abs(x) if x<0 else 0 for x in WT_wig]
        Mutant_wig_fwd = [abs(x) if x>0 else 0 for x in Mutant_wig]
        Mutant_wig_rev = [abs(x) if x<0 else 0 for x in Mutant_wig]
        printer_l(output_file_nom,ranked,genes_WT,genes_Mutant,WT_fwd,WT_rev,Mutant_fwd,Mutant_rev,GFF3,TU_fwd,TU_rev,CDS_fwd,CDS_rev,TU_wt,TU_mut,CDS_wt,CDS_mut,sequence,upstream_sequence,downstream_sequence,WT_lengths,Mutant_lengths,upstream_normalization,downstream_normalization,WT_wig_fwd,WT_wig_rev,Mutant_wig_fwd,Mutant_wig_rev)
    else:
        WT_wig,Mutant_wig = {},{}
        printer_s(output_file_nom,ranked,genes_WT,genes_Mutant,WT_fwd,WT_rev,Mutant_fwd,Mutant_rev,GFF3,TU_fwd,TU_rev,CDS_fwd,CDS_rev,TU_wt,TU_mut,CDS_wt,CDS_mut,sequence,upstream_sequence,downstream_sequence,WT_lengths,Mutant_lengths,upstream_normalization,downstream_normalization)
